...
class TagManager_Element extends TagManager
{
	...
	public static function tag_element_detail(FTL_Binding $tag)
	{
		...
		//Order elements
		$order = ($tag->getAttribute('order')) ? $tag->getAttribute('order') : FALSE;
		...
		$items = self::$ci->element_model->get_fields_from_parent($parent_type, $id_parent, Settings::get_lang(), $id_element_definition, $order);
	...
	public static function tag_element_item_field_value(FTL_Binding $tag)
	{
		...
		return self::output_value($tag, html_entity_decode($value));
	}
